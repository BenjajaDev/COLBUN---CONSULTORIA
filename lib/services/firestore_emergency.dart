...
class EmergencyContact {
  ...
  // Permite construir desde un mapa (e.g. assets/emergency_contacts.json)
  factory EmergencyContact.fromMap(Map<String, dynamic> data) {
    return EmergencyContact(
      id: data['id'] ?? '',
      keyWords: List<String>.from(data['key_words'] ?? const []),
      keyWordsEn: List<String>.from(data['key_words_en'] ?? (data['key_words'] ?? const [])),
      keyWordsPt: List<String>.from(data['key_words_pt'] ?? const []),
      name: data['name'] ?? '',
      nameEn: data['name_en'] ?? (data['name'] ?? ''),
      namePt: data['name_pt'] ?? '',
      phone: data['phone'] ?? '',
...
class EmergencyService {
  ...
  static const Set<String> _excludeWords = {
    ...
    'só perguntar', 'informação','perguntar', 'como te llamas', 'cuál es tu nombre', 'te llamo más tarde', 'llámame', 'no me llames',
  ...
  /// Carga contactos desde un archivo local (asset) como respaldo offline
  Future<void> loadEmergencyContactsFromAsset({String path = 'assets/emergency_contacts.json'}) async {
    try {
      ...
      _emergencyContacts = list
          ...
          .toList();
      print('📦 Emergency contacts loaded from asset: ${_emergencyContacts.length}');
    } catch (e) {
      print('❌ Error loading emergency contacts from asset: $e');
    }
  }
