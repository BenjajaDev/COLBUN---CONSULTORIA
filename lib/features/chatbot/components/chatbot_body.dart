...
// Método para abrir URLs mejorado con validación y mensajes multilingües
  void _launchUrl(String url, BuildContext context, {String language = 'es'}) async {
    try {
      // Validar que la URL no esté vacía
      if (url.isEmpty) {
        if (context.mounted) {
          _showErrorSnackbar(context, 'La URL está vacía', language);
        }
        return;
      }
      ...
      if (!uri.hasScheme || uri.host.isEmpty) {
        if (context.mounted) {
          _showErrorSnackbar(context, 'URL inválida: $url', language);
        }
        return;
      }
      ...
      if (!launched && context.mounted) {
        _showErrorSnackbar(context, 'No se pudo abrir: $url', language);
      }
    } catch (e) {
      if (context.mounted) {
        _showErrorSnackbar(context, 'Error al abrir el enlace', language);
      }
      debugPrint('❌ Error al abrir URL: $e');
    }
  }

  // Método auxiliar para mostrar mensajes de error multilingües
  void _showErrorSnackbar(BuildContext context, String error, String language) {
    final errorMessages = {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(errorMessages[language] ?? errorMessages['es']!),
        backgroundColor: Colors.red[700],
        duration: const Duration(seconds: 3),
        action: SnackBarAction(
          label: language == 'en' ? 'OK' : 'OK',
                      ...
                      child: ExcludeSemantics(
                        ...
                        children: [
                          ...
                          if (hasLink) ...[
                            ...
                            GestureDetector(
                              onTap: () {
                                _launchUrl(message['link'], context, language: messageLanguage);
                              },
                              child: Container(
