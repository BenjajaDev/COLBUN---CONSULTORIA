...
class _ChatbotScreenState extends State<ChatbotScreen>
  ...
  Future<void> _syncWithFirestore() async {
    if (!_isOnline || _currentConversationId == null) {
      print('⚠️ No hay conexión o conversationId para sincronizar');
      return;
    }
  ...
  void _uploadCachedMessagesInBackground(List<Map<String, dynamic>> cachedMessages) {
    if (!_isOnline || _currentConversationId == null) return; // NEW guard
  ...
  Future<void> _initializeConversation() async {
    ...
    // Si no hay internet, usa solo caché y/o mensaje de bienvenida
    if (!_isOnline) {
  ...
  Future<String?> addMessage({
    ...
    // Si no hay internet, no intentamos guardar en Firestore
    final shouldSaveRemote = saveToDatabase &&
        _isOnline && // NEW
        _currentConversationId != null &&
        text != null &&
        text.isNotEmpty;

    // Asegurar un ID local cuando no se guarda remoto
    newMessage['id'] ??= 'local_${DateTime.now().millisecondsSinceEpoch}'; // NEW
    ...
    try {
      ...
      if (mounted) {
        if (localResults.isNotEmpty) {
          ...
          final offlinePrefix = ChatbotStrings.get('fallback.offline_prefix', _currentLanguage);
  ...
  void _launchWhatsApp() async {
    ...
    if (!await launchUrl(whatsappUri, mode: LaunchMode.externalApplication)) {
      //===================================================
      // De momento no voy agregar if (!_isOnline) { // NEW if (!mounted) return; porque no sabemos si solo lo manejaremos offline o online
      //===================================================
